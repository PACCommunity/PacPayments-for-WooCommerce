...
if ( ! class_exists( 'WC_Gateway_PacPay' ) ) :

/**
 * PAC Payment Gateway
 *
 * Provides a PAC Payment Gateway
 *
 * @class    WC_Gateway_PacPay
 * @extends  WC_Payment_Gateway
 * @version  0.0.1
 * @package  PacPay
 * @author   "Black Carrot Ventures", The Pac Team
 */
class WC_Gateway_PacPay extends WC_Payment_Gateway {
    private static $currency = 'Pac';
    private static $currency_ticker_symbol = 'PAC';
    private static $currency_symbol = 'P';
    ...
    public function __construct() {
        ...
        $this->icon               = plugins_url('/assets/images/pac-39.png', DP_PLUGIN_FILE);
        $this->has_fields         = false;
        $this->method_title       = __('PacPay - Pay With $PAC', 'pacpay-woocommerce');
        $this->method_description = __('Pay with $PAC - Third Generation Cryptocurrency', 'pacpay-woocommerce');
    ...
    protected function check_valid() {
        if ( $this->enabled == 'no') {
            return __('Gateway is not enabled.', 'pacpay-woocommerce');
        }
        ...
        if ( 0 !== count($missing) ) {
            $msg = sprintf(
                esc_html__("Required extension(s) not loaded/enabled. Please enable '%s' PHP extension(s) on your WordPress server.", 'pacpay-woocommerce'),
                join(', ', $missing)
            );
            return $msg;
        }
        ...
        // Ensure xpub entered
        if ( ! $this->xpub_key ) {
            $msg = __("Please add a $PAC BIP32 extended public key (Launch your Electrum-$PAC wallet, Select Wallet->Master Public Keys)", 'pacpay-woocommerce');
            return $msg;
        }

        // Ensure valid xpub
        if ( ! CoinUtil::is_valid_public_xkey( $this->xpub_key ) ) {
            $msg = __("Invalid xpub key.", 'pacpay-woocommerce');
            return $msg;
        }
        ...
        $insight = new DP_Insight_API( $this->settings['insight_api_url'] );
        if ( ! $insight->is_valid() ) {
            $msg = __('Insight-API error in connection or network status', 'pacpay-woocommerce');
            return $msg;
        }
        ...
        try {
            ...
            $msg = __('Insight-API error.', 'pacpay-woocommerce');
            return $msg;
        }

        if ( $xpub_network != $insight_network ) {
            $msg = sprintf(
                esc_html__('Network discrepancy. Network for xpub key determined as %s, but Insight-API instance is running %s.', 'pacpay-woocommerce'),
                $xpub_network,
                $insight_network
            );
            return $msg;
    ...
    public function admin_notices() {
        ...
        if ( 0 !== count($missing) ) {
            $msg = sprintf(
                __("<strong>PacPayments for WooCommerce:</strong> Required extension(s) not loaded/enabled. Please enable '%s' PHP extension(s) on your WordPress server.", 'pacpay-woocommerce'),
                join(', ', $missing)
            );
        ...
        // Check for xpub key
        if ( ! $this->xpub_key ) {
            self::_admin_error( __("Please add a $PAC BIP32 extended public key (Launch your Electrum-$PAC wallet, Select Wallet->Master Public Keys)", 'pacpay-woocommerce') );
            return false;
        }
    ...
    public function init_form_fields() {
        $this->form_fields = array(
          'enabled' => array(
            ...
            'type' => 'checkbox',
            'label' => __( 'Enable PacPay', 'pacpay-woocommerce' ),
            'default' => 'yes'
          ),
          'title' => array(
            'title' => __( 'Title', 'pacpay-woocommerce' ),
            'type' => 'text',
            'description' => __( 'This controls the title which the user sees during checkout.', 'pacpay-woocommerce' ),
            'default' => __( 'PacPay - Pay with $PAC', 'pacpay-woocommerce' ),
            'desc_tip'    => true,
          ),
          'description' => array(
            'title'       => __( 'Description', 'woocommerce' ),
            'type' => 'text',
            'description' => __( 'Payment method description that the customer will see on your checkout page.', 'pacpay-woocommerce' ),
            'default' => __( 'Proceed to the next page to see payment details.', 'pacpay-woocommerce' ),
             'desc_tip'    => true,
          ),
          'xpub_key' => array(
            'title' => __( "$Pac BIP32 Extended Public Key", 'pacpay-woocommerce' ),
            'type' => 'textarea',
            'default' => '',
            'description' => __('1. Start <a href="https://github.com/PACCommunity/electrum-PAC/releases" target="_blank">Electrum-$PAC wallet</a> and get Master Public Key value from:<br>Wallet -> Master Public Key<br>2. Copy/paste the string starting with "xpub" into this field.', 'pacpay-woocommerce'),
            // 'desc_tip'    => true,
          ),
          'confirmations' => array(
            'title' => __( 'Confirmations required', 'pacpay-woocommerce' ),
            'type' => 'text',
            'description' => __( 'Number of confirmations required before payment is accepted.', 'pacpay-woocommerce' ),
            'default' => '3',
            // 'desc_tip'    => true,
          ),
          'exchange_multiplier' => array(
            'title' => __('Exchange rate multiplier', 'pacpay-woocommerce' ),
            'type' => 'text',
          ...
          'insight_api_url' => array(
            'title' => __('Insight API url', 'pacpay-woocommerce' ),
            ...
            'type' => 'text',
            'default' => 'http://explorer.paccoin.io',
            // 'desc_tip'    => true,
          ),
        );
    }
    ...
    public function admin_options() {
        echo '<h3>' . __('PacPay - Pay with $PAC', 'pacpay-woocommerce') . "</h3>\n";
        echo '<p>' . __('Enables direct $PAC payments for WooCommerce. <a href="https://paccoin.net"_blank">$PAC</a>, is a third generation cryptocurrency that is focused on become the
            most efficient and user-friendly cryptocurrency that is available globally.', 'pacpay-woocommerce') . "</p>\n";

        $error_message = $this->check_valid();

        if ( $error_message ) {
            $msg = sprintf(
                esc_html__('Pac payment gateway NOT operational. %s', 'pacpay-woocommerce'),
                $error_message
            );
            self::_OLD_admin_error( $msg );
        }
        else {
            self::_OLD_admin_success(__('PacPay is operational.','pacpay-woocommerce'));
        }
    ...
    public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
        ...
        if ( !$sent_to_admin && 'pacpay-woocommerce' === $order->payment_method && $order->has_status('pending') ) {
    ...
    public static function add_this_currency($currencies) {
        $currencies[self::$currency_ticker_symbol] = __( self::$currency, 'pacpay-woocommerce' );
        return $currencies;
    }
    ...
    protected static function _OLD_admin_success($msg) {
        echo '<p style="border:1px solid #d6e9c6;padding:5px 10px;font-weight:bold;color:#3c763d;background-color:#dff0d8;width: 400px;">' . $msg . '</p>';
    }
